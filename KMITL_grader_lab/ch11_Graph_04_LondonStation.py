town = {"Acton Town": ["Ealing Common", "South Ealing", "Turnham Green", "Chiswick Park"],
        ...
        "Oxford Circus": ["Picadilly Circus", "Regent's Park", "Tottenham Court Road", "Warren Street", "Bond Street",
                          "Green Park"],
...
def shortestPath(start, end, weightlst, visited, prevlst):
    # visited add
    visited.append(start)
    row = node.index(start)
    ...
    for index, col in enumerate(matrix[row]):
        if col != 0:
            if weightlst[row] + col < weightlst[index]:     # dijkstra relaxation
                weightlst[index] = weightlst[row] + col
                prevlst[index] = start
    # end relaxation
    ...
    # recur or not found...
    if minIndex == 999:     # can't go
        return
    else:
        shortestPath(node[minIndex], end, weightlst, visited, prevlst)
...
def printShortest(start, end, prevlst):
    ...
    while currentNode != start:
        ...
        currentIndex = node.index(currentNode)
        currentNode = prevlst[currentIndex]
    else:
        stack.append(currentNode)
...
# preparation for dijkstra shortest path (create with adjacent matrix)
node = []
for key in town.keys():  # add everything to node
    node.append(key)

matrix = [[0 for i in range(len(node))] for i in range(len(node))]

for startIndex, valuelst in enumerate(town.values()):
    for end in valuelst:
        endIndex = node.index(end)  # get index of end station
        matrix[startIndex][endIndex] = 1  # push in matrix

#print(node)
#print(matrix)


# init dijkstra requirement
visited = []
weightlst = [999 for i in range(len(node))]
prevlst = [-1 for i in range(len(node))]

## INPUT HERE
startStation, endStation = input('Enter : ').split(',')
print(f'From: {startStation}')
print(f'To: {endStation}')
print('Searching shortest route... This may take a while...')
print('Suggested Route: ')

weightlst[node.index(startStation)] = 0

shortestPath(startStation, endStation, weightlst, visited, prevlst)
printShortest(startStation, endStation, prevlst)
